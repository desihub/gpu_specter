...
Optional Inputs:
    ...
    '''
    #!!! this section should be double-checked, preferably by stephen!
    #lets get these from our specter psf since we no longer use specter
    ...
    #!!! this seciton should be double checked by stephen
    wavemin, wavemax = wavelengths[0], wavelengths[-1] #here we use the values from wavelengths instead of the psf itself
    ...
    #this check fails-- is our grid nonlinear?
    ...
    #!!! this section should be checked by stephen
    #are wavemin and wavemax equivlent to psf.wmin and psf.max?
    ...
    #are the saved values from our original specter psf correct here?
    wmid = (wavemin + wavemax) / 2.0
    spotsize = psf_pix.shape
    ...
    #!! this section should be double checked by stephen
    #manually set psferr
    psferr = 0.01 #double check!

    #!!! this section should be checked by stephen
    #since we've removed subbundles, set this up manually
    speclo = specmin
    ...
    #cache spots for entire bundle
    spots = cache_spots(nspec, nwave, p, wavelengths)
    #also need the bottom corners for projection_matrix
    xc = np.floor(p['X'] - p['HSIZEX']//2).astype(int)
    yc = np.floor(p['Y'] - p['HSIZEY']//2).astype(int)
    corners = (xc, yc)
    #need to keep track of the indicies for projection_matrix
    ispecmin = speclo
    ispecmax = spechi
    ...
    for iwave in range(0, tws, wavesize):
        ...
        #can't use psf.xyrange like specter does, do it ourselves
        ny, nx = spots.shape[2:4]
        xlo = np.min(xc[0:spechi-speclo, iwavemin:iwavemax])
        xhi = np.max(xc[0:spechi-speclo, iwavemin:iwavemax]) + nx
        ylo = np.min(yc[0:spechi-speclo, iwavemin:iwavemax])
        yhi = np.max(yc[0:spechi-speclo, iwavemin:iwavemax]) + ny
        ...
        results = \
            ex2d_patch(subimg, subivar, spots, corners, iwave,
...
def ex2d_patch(image, ivar, spots, corners, 
    ...
    #the issue here is that A and W are different sizes
    #A.shape is (20223, 1675)
    #W.shape is (16168, 16168)
